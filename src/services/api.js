...
export const getVendors = (status) => api.get('/vendors/', { params: { status } });
export const getVendorById = (id) => api.get(`/vendors/${id}`);
export const updateVendor = (id, updates) => api.patch(`/vendors/${id}`, updates);
...
export const getMenuItems = (vendorId) => api.get('/menu/', { params: { vendor_id: vendorId } });
...
export const updateOrderStatus = (id, updates) => api.patch(`/orders/${id}`, updates);
...
export const updateVendorProfile = (vendorId, updates) => api.patch(`/vendors/${vendorId}`, updates);

// Vendor Order Management (Backend)
export const getVendorOrdersBackend = (vendorId) => api.get('/orders/', { params: { vendor_id: vendorId } });
export const getVendorOrderTracking = (orderId) => api.get(`/orders/${orderId}/tracking`);
export const addOrderTrackingUpdate = (orderId, trackingData) => api.post(`/orders/${orderId}/tracking`, trackingData);
...
export const createUserProfileIfNotExists = async (userId, userEmail, userData) => {
...
export const sendMessage = async (conversationId, senderId, content, type = 'text', url) => {
  const { data, error } = await supabase
    .from("messages")
    .insert({
...
export const createConversation = async (participants, type = 'direct', metadata = {}) => {
...
export const getTopUsers = async (limit = 5) => {
  ...
  data.forEach(order => {
    ...
    if (!counts[id]) { counts[id] = { name: order.users ? `${order.users.firstname} ${order.users.lastname}` : 'Unknown', orders: 0 }; }
    counts[id].orders++;
  });
  return { data: Object.values(counts).sort((a, b) => b.orders - a.orders).slice(0, limit), error: null };
};
...
export const getPopularItems = async (limit = 3) => {
  ...
  data.forEach(item => {
    ...
    if (!id) return;
    if (!counts[id]) { counts[id] = { name: item.menu_items?.name || 'Unknown', category: item.menu_items?.category || 'General', count: 0 }; }
    counts[id].count++;
  });
  return { data: Object.values(counts).sort((a, b) => b.count - a.count).slice(0, limit), error: null };
};
