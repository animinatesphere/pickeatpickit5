...
export default function ChatApp() {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full overflow-hidden bg-white md:my-4 md:rounded-2xl md:shadow-xl md:border md:border-gray-200">

        {/* Sidebar */}
        <div className={`flex-col w-full md:w-80 lg:w-96 border-r border-gray-200 bg-white ${activeView === "chat" ? "hidden md:flex" : "flex"}`}>
          <div className="p-5 border-b border-gray-100 bg-white sticky top-0 z-10">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 bg-gray-100 rounded-xl text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/30 transition-all"
              />
              {searchQuery && (
                  ...
                  >
                    {searching ? (
                      ...
                      searchResults.map((result) => (
                        ...
                        >
                          <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center font-semibold text-green-700 flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <h3 className="font-semibold text-gray-800">No Chats Yet</h3>
              ...
              conversations.map((conv) => (
                <motion.div
                  ...
                  className={`group flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${activeChat?.id === conv.id ? 'bg-green-600 text-white' : 'hover:bg-gray-50'}`}
                >
                  <div className="relative flex-shrink-0">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-semibold text-lg ${activeChat?.id === conv.id ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'}`}>
                      {conv.name.charAt(0).toUpperCase()}
                    </div>
                    {conv.online && (
                      <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 ${activeChat?.id === conv.id ? 'bg-green-300 border-green-600' : 'bg-green-500 border-white'}`} />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <h3 className={`font-semibold text-sm truncate ${activeChat?.id === conv.id ? 'text-white' : 'text-gray-900'}`}>{conv.name}</h3>
                    ...
                    <p className={`text-xs truncate ${activeChat?.id === conv.id ? 'text-green-100' : conv.unread > 0 ? 'font-semibold text-gray-700' : 'text-gray-500'}`}>{conv.message}</p>
                  </div>
                </motion.div>
              ))
            )}
        ...
        <div className={`flex-1 flex flex-col bg-gray-50 h-full overflow-hidden ${activeView === "messages" ? "hidden md:flex" : "flex"}`}>
          {activeChat ? (
            <>
              <div className="px-4 py-3 sm:px-6 sm:py-4 bg-white border-b border-gray-200 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  <button onClick={() => setActiveView("messages")} className="md:hidden p-2 bg-gray-100 rounded-xl text-gray-500">
                    <ArrowLeft size={20} />
                  </button>
                ...
                <div className="flex items-center gap-1">
                  <button className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 hover:bg-green-100 hover:text-green-600 transition-all">
                    <Phone size={18} />
                  </button>
                  <button className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 hover:bg-gray-200 transition-all">
                    <MoreVertical size={18} />
                  </button>
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`max-w-[75%] sm:max-w-[65%]`}>
                        <div className={`px-4 py-3 rounded-2xl ${isMe ? 'bg-green-600 text-white rounded-br-sm' : 'bg-white text-gray-900 rounded-bl-sm shadow-sm border border-gray-100'}`}>
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                  ...
                  <div className="relative flex-1">
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gray-100 rounded-xl text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/30 transition-all"
                    />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mb-6">
                <MessageCircle size={40} className="text-green-600 opacity-50" />
              </div>
              ...
              <div className="mt-6 p-4 bg-green-50 rounded-xl w-full max-w-sm">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-green-600 rounded-lg flex items-center justify-center text-white flex-shrink-0">
                    <User size={18} />
                  </div>
                  <p className="text-xs font-medium text-green-700 text-left">Type a phone number in the search box to find someone</p>
                </div>
              </div>
            </div>
          )}
