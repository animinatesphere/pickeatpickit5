...
export default function HeroFoodCarousel() {
  ...
  useEffect(() => {
    const fetchHeroItems = async () => {
      try {
        setLoading(true);
        const data = await backendAuthService.getMenuItems(4);
        // Sort by highest discount client-side (mirrors the old Supabase order)
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
          ...
          <div className="space-y-8 order-2 lg:order-1 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                to="/market"
                className="bg-green-600 hover:bg-green-700 text-white font-bold py-4 px-8 rounded-xl text-lg flex items-center gap-3 transition-all transform hover:scale-105 shadow-xl"
              >
          ...
          <div className="relative order-1 lg:order-2">
            ...
            <button
              ...
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-green-600 p-3 rounded-full shadow-lg"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>
            <button
              onClick={goToNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-green-600 p-3 rounded-full shadow-lg"
            >
