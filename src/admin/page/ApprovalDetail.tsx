...
const riderSections: [string, [string, string][]][] = [
  ["Application", [["status", "Approval status"], ["is_active", "Account active"], ["profile_completed", "Profile complete"], ["created_at", "Submitted"], ["id", "Rider ID"], ["user_id", "User ID"]]],
  ...
  ["Vehicle and work", [["vehicle_type", "Vehicle type"], ["vehicle_brand", "Vehicle brand"], ["plate_number", "Plate number"], ["previous_work", "Previous work"], ["work_duration", "Work duration / availability"], ["delivery_range", "Delivery range (km)"], ["referral_code", "Referral code"]]],
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {sections.map(([heading, fields]) => <DetailSection key={heading} heading={heading} fields={fields} record={record} />)}
      </div>

      {kind === "rider" && <>
        <MediaSection title="Submitted documents" items={[["Profile / face photo", record.profile_image, "image"], ["Driver's licence", record.license_image, "image"]]} />
      ...
      {kind === "vendor" && <>
        <MediaSection title="Submitted documents and media" items={[["Logo", record.logo_url, "image"], ["Cover image", record.cover_url, "image"], ["Storefront photo", record.store_front_picture_url, "image"], ["Face verification video", record.face_video_url, "video"]]} />
...
function DetailSection({ heading, fields, record }: { heading: string; fields: [string, string][]; record: Row }) {
  return <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><h2 className="font-black text-slate-900">{heading}</h2><div className="mt-4"><FieldGrid record={record} fields={fields} /></div></section>;
}

function FieldGrid({ record, fields }: { record: Row; fields: [string, string][] }) {
  return <dl className="grid gap-x-5 gap-y-4 sm:grid-cols-2">{fields.map(([key, label]) => <div key={key} className={key.includes("description") || key.includes("address") ? "sm:col-span-2" : ""}><dt className="text-[10px] font-black uppercase tracking-wide text-slate-400">{label}</dt><dd className="mt-1 break-words text-sm font-semibold text-slate-700">{formatValue(record[key])}</dd></div>)}</dl>;
}
